...
class StreamReportsController extends Controller
{
    public function index()
    {
        ...
        $streamreports = StreamReport::latest()->limit(100)->get();

        return view('core.reports.streams.index', compact('page', 'streamreports'));
    }
    ...
    public function search(Request $request)
    {
        ...
        [
          'search.required'     => 'You need to search an stream report.',
        ]);

        $query                  = $request->input('search');


        $streamreports = StreamReport::where('name', 'LIKE', '%' . $query . '%')
            ->get();

        if(count($streamreports)==1)
        {
            $tense              = 'is';
            $result             = 'result.';
        }
        ...
        $message                = 'There '. $tense . ' ' . count($streamreports) . ' streamreport '. $result;
    ...
    {
    	$page = 'Create Stream Report';

        $exams = Exam::get();

    	return view('core.reports.streams.create', compact('page', 'exams'));
    }
    ...
    public function postcreate(Request $request)
    {
        ...
        $streamreport = StreamReport::create([
            'name'            => $name,
            'from_user'       => $from_user
        ]);
        ...
        foreach($exams as $exam)
        {
            ...
            $streamreport->assignExam($examSelected);
        }

        $message = 'Stream Report created successfully!';

        return redirect('stream-reports')->with('success', $message);
    }
    ...
    public function attachexam($id)
    {
        ...
        $streamreport = StreamReport::whereId($id)->first();

        $exams = Exam::get();

        return view('core.reports.streams.attach', compact('page', 'streamreport', 'exams'));
    }
    ...
    public function postattachexam(Request $request, $id)
    {
        ...
        foreach($exams as $exam)
        {
            ...
            if(!$streamreport->hasExam($examSelected->id)){
        ...
        return redirect()->route('view-stream-report', compact('id'))->with('success', $message);
    }


    public function view($id)
    {
        $page = 'View Stream Report';

        $streamreport = StreamReport::whereId($id)->first();

        return view('core.reports.streams.view', compact('page', 'streamreport'));
    }

    public function edit($id)
    {
        $page = 'Edit Stream Report';

        $streamreport = StreamReport::whereId($id)->first();

        return view('core.reports.streams.edit', compact('page', 'streamreport'));
    }
    ...
    public function postedit(Request $request, $id)
    {
        ...
        $streamreport = StreamReport::whereId($id)->update([
            'name'            => $name,
            'from_user'       => $from_user
        ]);

        $message = 'Stream Report updated successfully!';
    ...
    public function detach($sr_id, $id)
    {
        ...
        return view('core.reports.streams.detach', compact('page', 'exam', 'sr_id', 'id'));
    }

    public function postdetach($sr_id, $id)
    {   
        $streamreport     = StreamReport::whereId($sr_id)->first();

        $examSelected     = Exam::whereId($id)->first();
        
        $streamreport->removeExam($examSelected);

        $message = 'Exam detached successfully!';

        return redirect()->route('view-stream-report', compact('sr_id'))->with('success', $message);
    }
    ...
    public function delete($id)
    {
        ...
        return view('core.reports.streams.delete', compact('page', 'streamreport'));
    }
    ...
    public function postdelete($id)
    {
        ...
        $message = 'Stream Report deleted successfully!';
    ...
    {    
        ...
        //Get 1st exam in stream report
        $streamreport_exam = $streamreport->exams->first();

        $streamreport_stream_id = $streamreport_exam->stream_id;
        $class_name           = $streamreport_exam->stream->name;
        $streamreport_name    = $streamreport_exam->name . ' Exam, ' . $streamreport_exam->period . ', ' . $streamreport_exam->year;

        $date = Carbon::today()->format('jS F\\, Y');


        Excel::create('Grade Report', function($excel) use($id, $school_name, $school_address,  $class_name, $streamreport_name, $streamreport_stream_id, $date){

            $excel->sheet('Grade Report', function($sheet) use($id, $school_name, $school_address, $class_name, $streamreport_name, $streamreport_stream_id, $date){
                ...
                $sheet->cell('A4', function($cell) use($class_name) {
                    $cell->setValue('Class Report, '. $class_name);
                    $cell->setFontWeight('bold');
                    $cell->setAlignment('center');
                    $cell->setFontSize(10);
                });

                $sheet->cell('A5', function($cell) use($streamreport_name) {
                    $cell->setValue($streamreport_name);
                    $cell->setFontWeight('bold');
                    $cell->setAlignment('center');
                    $cell->setFontSize(10);
                });
                ...
                $students = User::whereHas(
                    ...
                    'grades.assessment.exam', function($q) use($streamreport_stream_id){
                        $q->where('stream_id', $streamreport_stream_id);
                    }
                )->get();
                ...
                {      
                    ...
                    $students_marks[$i]['student_name'] = $student->name;
